...
1. Create a new folder `env-info` inside your local repository `<user>.github.io`. 

Now, for every week's assignment you'll create a new Rmarkdown file inside the `env-info` folder of your `<user>.github.io` repo. This week create one called `wk05_ggplot.Rmd` with File, New File..., R Markdown...

We'll be able to grade assignments, because we have a roster of students by Github username, and so presume we'll find your assignment by visiting `http://<user>.github.io/env-info/wk05_dplyr.html`, eg `http://bbest.github.io/env-info/wk05_dplyr.html`. 

## joining data

For this assignment, you can similarly ....

### setup

```{r setup, echo=F}
# NOTE: This chunk sets the default to eval=F for all following chunks.
#       Am just using this for instructions. Please exclude in your assignment.
