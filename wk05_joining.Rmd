...
```{r setwd students}
# set working directory if has env-info_hw directory and at R Console (vs knitting)
if ('env-info_hw' %in% list.files() & interactive()){
    setwd('env-info_hw')
}

# ensure working directory is students
if (basename(getwd()) != 'env-info_hw'){
  stop(sprintf("WHOAH! Your working directory is not in 'env-info_hw'!\n   getwd(): %s", getwd()))
}
```
