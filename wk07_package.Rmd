...
The R chunks below will walk you through creation of a package called `mypackage` with the following elements:

- **functions** in `R/*.R` scripts. Functions are written here with input arguments and return objects.
- **documentation** in `man/*.Rd`. Documentation is automatically generated from the `roxygen2` comments in the R scripts. Help is then available from the console via `?add` (eg for `add()` function).
...
- **vignettes** in `vignettes/`. Vignettes are a long form of documentation that allow you to walk the user through a larger amount of analysis, especially useful for showing relatedness between functions are showing plots, tables and richer documentation since it is rendered as Rmarkdown to HTML.
...
If you use `create()`, you should open the `mypackage/mypackage.Rproj` to open RStudio with that working directory. Notice also an extra **Build** tab appears specifically for building packages.

See more:

- [rstudio-IDE-cheatsheet](.refs/cheatsheets/rstudio-IDE-cheatsheet.pdf): Package Structure, Setup, Write code in R/
...
Notice how `document()` creates the [`man/add.Rd`](https://github.com/bbest/bbest.github.io/blob/master/mypackage/R/add.R) (and any other `man/*.Rd`) files and updates `DESCRIPTION` and `NAMESPACE`.
...
- [rstudio-IDE-cheatsheet](.refs/cheatsheets/rstudio-IDE-cheatsheet.pdf): Write code in R/, Document in man/, Organize in NAMESPACE, Setup in DESCRIPTION
...
- [rstudio-IDE-cheatsheet](.refs/cheatsheets/rstudio-IDE-cheatsheet.pdf): Add data in data/
...
- [rstudio-IDE-cheatsheet](.refs/cheatsheets/rstudio-IDE-cheatsheet.pdf): Test in tests/
...
- [rstudio-IDE-cheatsheet](.refs/cheatsheets/rstudio-IDE-cheatsheet.pdf): Teach in vignettes/
...
Notice how the first argument is of the form `<owner>/<repository>` and the subdirectory of `mypackage`. For more serious package development, I recommend you dedicate an entire repository so you don't have to use the `subdir` argument.
